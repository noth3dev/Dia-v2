    ...
    function fetchMealData(date) {
        ...
        function displayMealData(mealData) {
            ...
            // Insert meal data into the respective lists
            ...
            // Display the meal date and day of the week
        ...
        // Function to change the date and fetch new meal data
        let currentDate = getKoreaDate(); // 한국 시간을 기준으로 초기화

    function changeDate(offset) {
        currentDate.setDate(currentDate.getDate() + offset); // 날짜를 변경
        fetchMealData(currentDate); // 변경된 날짜로 급식 데이터를 다시 가져옴
    }

    // 페이지 로드 시 기본적으로 한국 날짜의 급식 데이터를 가져옴
    fetchMealData(currentDate);

        function changeDate(offset) {
            currentDate.setDate(currentDate.getDate() + offset);
            fetchMealData(currentDate); // 변경된 날짜로 데이터를 가져옴
        }

            // Function to randomize seating arrangement
            const students = [
                "고하은", "권순모", "김규빈", "김무준", "김민찬", 
                "김세준", "김소은", "김재민", "김준원", "마현진", 
                "박현준", "백재원", "여지민", "오민혜", "우리나", 
    ...
    function addTest() {
        const adminKey = prompt('어드민 키를 입력하세요:');
        
        
        if (adminKey === 'admin123') {
            const testDate = prompt('추가할 평가의 날짜를 YYYY-MM-DD 형식으로 입력하세요:');
            const testPeriod = prompt('추가할 평가의 교시를 입력하세요:');
            const testName = prompt('추가할 평가의 이름을 입력하세요:');

            if (testDate && testPeriod && testName) {
                
                if (!events[testDate]) {
                    events[testDate] = []; 
                }
                events[testDate].push({ name: testName, period: parseInt(testPeriod, 10) });
                ...
                alert('평가가 성공적으로 추가되었습니다.');
            } else {
                alert('입력 값이 올바르지 않습니다.');
            }
        } else {
            alert('어드민 키가 잘못되었습니다.');
        }
    }

    
    document.getElementById('addTestButton').addEventListener('click', addTest);
...
function placeBet(choice) {
    ...
    const result = isEven ? '짝 (Even)' : '홀 (Odd)';
